...
def testing(synaptic_weights):
    ...
    f = open('normal.txt')
    for line in f:
        if number_line > 5000 and number_line < 7000:
...
def training():
    ...
    f1 = open('DOS/neptune.txt', 'r')
    f2 = open('normal.txt', 'r')
    F = [f1, f2]
    N = [7000, 3500]
    temp = 1
    m = 1
    training_outputs_1 = np.ones((7001, 1))
    training_outputs_2 = np.zeros((3500, 1))
    training_outputs = np.vstack([training_outputs_1, training_outputs_2])

    synaptic_weights = (2 * np.random.random((10, 1)) - 1) * 0.5
